...
import { createAttendance, getStaffAttendance, getMonthlyStaffAttendance, leaveRequest } from "@/api/attendance.api";
...
const AttendanceManagement = () => {
  ...
  const { data: todaySummary, refetch: refetchTodaySummary, isPending: isTodaySummaryPending } = useQuery({
    ...
    enabled: Boolean(user?.restaurantId._id && startDateOfMonth && endDateOfMonth),
  });
  ...
  const LeaveApplicationModal = ({ leaveImagePreview, setLeaveImagePreview }: { leaveImagePreview: string | null; setLeaveImagePreview: (preview: string | null) => void }) => {
    ...
    const handleOnChange = (dates: [Date | null, Date | null]) => {
      ...
      // Always update both fields, but only if start date exists
      if (start) {
        // Use setValue from useForm to update the form values
        setLeaveValue("fromDate", formatDate(start));
        setLeaveValue("toDate", formatDate(end || start)); // If no end date, use start date

        // If both dates are selected, we can log or perform additional actions
        if (start && end) {
        }
      }
    };
    ...
    return (
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 gap-4">
            <div className="flex flex-col gap-2 w-full">
              ...
              <div className="relative w-full" onClick={() => setIsCalendarOpen(true)}>
                <DatePicker
                  id="leave-date"
                  selectsRange
                  startDate={startDate}
        ...
        <div>
          ...
          />
          {leaveErrors.reason && (
            <p className="mt-1 text-xs text-red-500">{String(leaveErrors.reason.message)}</p>
          )}
        </div>
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => setIsApplyingLeave(false)} disabled={isLeavePending}>
            Cancel
          </Button>
          <Button onClick={handleLeaveSubmit(onSubmitLeave)} disabled={isLeavePending}>
            {isLeavePending && (
              <span className="mr-2 h-4 w-4 border-2 border-white border-t-transparent rounded-full inline-block animate-spin" />
            )}
            ...
            attendanceData?.map((record) => (
              ...
              >
                <div className="col-span-2 flex items-center space-x-3">
                  <div
                    className={`p-2 rounded-lg ${record.status === "present"
